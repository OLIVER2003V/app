...
export default function Home() {
  ...
  useEffect(() => {
    api.get("posts/", { params: { limit: 4 } }).then(({ data }) => setPosts(data?.results || data || [])).catch(() => {});
    api.get("reviews/").then(({ data }) => setReviews((data?.results || data || []).slice(0, 5))).catch((err) => console.error("Error al cargar opiniones:", err));
    fetch("/ruta.json").then((r) => (r.ok ? r.json() : Promise.reject("No se pudo cargar la ruta."))).then((geo) => {
  ...
  return (
    <div className="home">
      {/* ▼▼▼ El carrusel se coloca dentro del contenedor principal ▼▼▼ */}
        ...
        <section className="home-section">
          <div className="home-container">
            ...
            <div className="featured-posts-grid">
              {posts.map((p) => (
                <div key={p.id} className="postCard" onClick={() => openPost(p)} title={p.title}>
      ...
      <section className="home-section alt-bg">
        <div className="home-container">
          ...
          <div className="quick__grid">
            <Link className="card" to="/como-llegar"><span className="card__icon">🧭</span><h3>¿Cómo llegar?</h3><p>Ruta interactiva y opciones de transporte.</p></Link>
            <Link className="card" to="/informacion"><span className="card__icon">ℹ️</span><h3>Información</h3><p>Horarios, tarifas y contacto.</p></Link>
            <a className="card" href="https://chat.whatsapp.com/EpzISekSBCe08kJh9LsQpx" target="_blank" rel="noreferrer"><span className="card__icon">💬</span><h3>WhatsApp</h3><p>Atención turística directa.</p></a>
          </div>
        </div>
      </section>
      {reviews.length > 0 && (
            ...
            <div className="testimonials-grid">
              {reviews.map((r) => (
                <div key={r.id} className="testimonial-card">
                  {r.photo && <img src={r.photo} alt={`Opinión de ${r.author_name}`} className="testimonial-photo" />}
                  <p className="testimonial-comment">"{r.comment}"</p>
                  <div className="testimonial-footer">
                    <div className="testimonial-author-info">
                      <span className="testimonial-author">{r.author_name}</span>
                      <span className="testimonial-rating">{[...Array(r.rating)].map((_, i) => (<Star key={i} />))}</span>
                    </div>
                    {r.place_name && r.place_slug && <Link to={`/places/${r.place_slug}`} className="testimonial-place">en {r.place_name}</Link>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <section id="mapa" className="home-section">
        <div className="home-container">
          ...
          <div className="home-map-container">
            {mapError ? <div className="map-error-message">{mapError}</div> : <InteractiveTrailMap trailData={trail} />}
          </div>
