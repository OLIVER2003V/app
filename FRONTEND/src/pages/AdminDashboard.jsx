...
// Un único dashboard para admin/editor: antes existían dos componentes casi
// idénticos (AdminDashboard y EditorDashboard) que se desincronizaban en cada
// cambio. Ahora los permisos de backend son los mismos para ambos roles en
// casi todo el contenido, así que comparten la misma pantalla; solo la
// gestión de usuarios queda oculta para editores (el backend ya la protege
// con IsAdmin de todas formas).
export default function AdminDashboard() {
  ...
  // Conteos reales para las tarjetas de resumen (antes quedaban fijas en "—")
  const [stats, setStats] = useState({ posts: null, places: null, events: null, reviews: null });
  const [statsLoading, setStatsLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    const endpoints = { posts: "posts/", places: "places/", events: "events/", reviews: "moderation/reviews/" };

    const loadStats = async () => {
      setStatsLoading(true);
      const results = await Promise.allSettled(
        Object.values(endpoints).map((path) => api.get(path))
      );
      if (cancelled) return;
      ...
      Object.keys(endpoints).forEach((key, i) => {
        ...
        if (result.status === "fulfilled") {
          ...
          counts[key] = Array.isArray(data) ? data.length : (data?.count ?? null);
        } else {
          counts[key] = null;
        }
      });
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <DashboardStat label="Publicaciones" value={statValue("posts")} hint="Artículos activos" />
            <DashboardStat label="Lugares" value={statValue("places")} hint="Destinos registrados" />
            <DashboardStat label="Eventos" value={statValue("events")} hint="En calendario" />
            <DashboardStat label="Reviews" value={statValue("reviews")} hint="Pendientes de revisión" />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <DashboardCard
              ...
              desc="Controla las imágenes y videos destacados del carrusel de inicio."
            >
              <div className="mt-4">
                <DashboardButton to="/admin/gallery" variant="primary" className="w-full justify-center shadow-lg shadow-indigo-500/20">
                  Gestionar Galería
                </DashboardButton>
              </div>
            </DashboardCard>
            ...
            >
              <div className="flex flex-col gap-3 mt-4">
                ...
                <DashboardButton variant="secondary" to="/admin/posts" className="w-full justify-center">Ver Todo</DashboardButton>
              </div>
            </DashboardCard>
            ...
            <DashboardCard
              ...
              desc="Administra la información de cascadas, senderos y zonas, incluyendo sus fotos."
            >
              <div className="flex flex-col gap-3 mt-4">
                <DashboardButton to="/admin/places" variant="primary" className="w-full justify-center shadow-lg shadow-indigo-500/20">
                  Administrar Lugares
                </DashboardButton>
                <DashboardButton variant="secondary" to="/places" className="w-full justify-center">Ver en el sitio</DashboardButton>
              </div>
            </DashboardCard>
            ...
            >
              <div className="flex flex-col gap-3 mt-4">
                <DashboardButton to="/admin/create-event" variant="primary" className="w-full justify-center shadow-lg shadow-indigo-500/20">
                  Administrar Eventos
                </DashboardButton>
                <DashboardButton variant="secondary" to="/events" className="w-full justify-center">Ver Calendario</DashboardButton>
              </div>
            </DashboardCard>
            ...
            {/* Tarjeta: Información del sitio */}
            <DashboardCard
              title="Información del Sitio"
              icon={<Info className="h-6 w-6" />}
              desc="Horarios, tarifas, reglas del parque y qué llevar — lo que ven los turistas en Inicio, Información y Cómo Llegar."
            >
              <div className="mt-4">
                <DashboardButton to="/admin/site-info" variant="primary" className="w-full justify-center shadow-lg shadow-indigo-500/20">
                  Editar Información
                </DashboardButton>
              </div>
            </DashboardCard>
            ...
            >
              <div className="flex flex-col gap-3 mt-4">
                <DashboardButton to="/admin/contactos/nuevo" variant="primary" className="w-full justify-center shadow-lg shadow-indigo-500/20">
                  + Nuevo Contacto
                </DashboardButton>
                <DashboardButton variant="secondary" to="/admin/contactos" className="w-full justify-center">Ver Lista</DashboardButton>
              </div>
            </DashboardCard>

            {/* Tarjeta 7: Usuarios (solo admin) */}
            {isAdmin && (
              <DashboardCard
                title="Usuarios y Roles"
                icon={<Users className="h-6 w-6" />}
                desc="Da de alta colaboradores y decide quién puede editar el contenido."
              >
                <div className="mt-4">
                  <DashboardButton to="/admin/users" variant="primary" className="w-full justify-center shadow-lg shadow-indigo-500/20">
                    Gestionar Usuarios
                  </DashboardButton>
                </div>
              </DashboardCard>
